...
import ch.hsr.ifs.petmanager.PetManager;
import ch.hsr.ifs.ui.listeners.PeopleTableSelectionListener;
import ch.hsr.ifs.ui.models.JobListModel;
import ch.hsr.ifs.ui.models.PeopleTableModel;
import ch.hsr.ifs.ui.models.PetsTableModel;
import ch.hsr.ifs.ui.renderers.PetIconsTableCellRenderer;
			...
			removePersonButton.addActionListener(new AbstractAction() {
				public void actionPerformed(ActionEvent evt) {
					if (peopleTable.getSelectedRowCount() > 0) {
						...
						((PeopleTableModel) getPeopleTable().getModel()).removePersonAtRow(selectionModel);
					}
				}
			});
	...
	private final class RemovePetButtonListener extends AbstractAction {
		public void actionPerformed(ActionEvent evt) {
			if (petsTable.getSelectedRowCount() > 0) {
				...
				((PetsTableModel) getPetsTable().getModel()).removePetAtRow(selectionModel);
